...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.65\textwidth]{Figures/gpuOptimization.png}
		\smallskip
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.77 \textwidth]{Figures/speedup.png}
		\smallskip
...
The Tesla K20m was the only GPU which in every code modification it did not lose performance over the course of the optimization process. However, the other GPUs drop performance in the stream optimization stage. The stream process is where each kernel was divided into three separated kernels. Doing this we were able to calculate the x, y, z coordinates independently. In addition, this enable room to implement shared memory across possible kernels. As the figure \ref{tab:gpus} illustrates the Tesla K20m is the only GPU card with CC of 3.5. This is important to mention, because the CC 3.5 has access to Hyper-Q. Moreover, this improvement can synchronize automatically concurrent kernels for them to launch in a parallel manner. 

The SOA optimization overall improved dramatically the performance of the application, obtaining a 1.2x - 2.0x speed up in all GPU cards, figure \ref{fig:speedup} \ref{fig:speeduplast}.

\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.7\textwidth]{Figures/speed.png}
		\smallskip
