...
Consider a regular, differentiable one-dimension  scalar function $f(x)$ sampled at regular intervals, a, see figure  \ref{fig:bound}. Second order Taylor expansion readily tiles expressions for the first and seconds central derivates that are widely used in numerics, namely $\frac{df}{dx} = \frac{f_{i+1} - f_{i-1}}{2a}$ and $\frac{d^2f}{dx^2} = \frac{f_{i+1} - 2f_i + f_{i-1}}{a^2}$. Demonstration of such equations reference \cite{methods}.
...
In both cases, and second derivatives and fully determined provided $f^{(1)}(x_R)$ be known along the boundary. For further reference and demonstration of such equations and matrices please read \cite{methods}. For the CUDA code implementation of the laplacian boundaries conditions please read Chapter \ref{Implementation of Domain Wall Dynamics under Nonlocal STT}.
