...
As said the GPU exceeds the CPU in calculations per second FLOPS with a low energy consumption. However the GPU cannot execute a complex application like a web browser. Because of GPU architecture, which is designed to launching small amounts of data in parallel with only several instructions, while the CPU can perform a single operation on comparatively large set of data with only one instruction.
