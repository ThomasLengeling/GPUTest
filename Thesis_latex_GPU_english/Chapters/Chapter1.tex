...
The biggest difference between the CPU code and the GPU code is the while loop and the kernel. The CPU code has it only have one thread there is only one loop, however if the CPU has multiples thread, an extra loop is requiered for each aditional thread. Bases on this idea, the GPU kernel lauches to code thoughout th threads of the device chip. 
