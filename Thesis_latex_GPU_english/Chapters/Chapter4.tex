...
In other words the global memory resides on the device, and it can be anything from 1 byte to 8GB, depends on the GPU RAM available. Furthermore, the memory is visible to all the threads of the grid. Any thread can read and write to any location of the global memory, The memory is always allocated with the keyword \textit{cadaMalloc}. In addition, the global memory can only be used by passing to the kernels call the keyword \twoline global \twoline \cite{design}.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.8\textwidth]{Figures/visualgraph.png}
		\rule{35em}{0.2pt}
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.33\textwidth]{Figures/visual01.png}
		\rule{35em}{0.5pt}
